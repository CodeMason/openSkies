...
import com.github.skittishSloth.openSkies.engine.ui.dialog.QuestDialog;
import com.github.skittishSloth.openSkies.engine.ui.hud.QuestTracker;
...
public class LocalScreen extends AbstractScreen {
    ...
    public LocalScreen(final OpenSkies game, final MapAssets mapAssets) {
        ...
        this.questState = playerState.getQuestState();
        ...
        uiCamera.update();
        dialog = new QuestDialog(getSkin());
        ...
        questTracker = new QuestTracker(getSkin(), playerState.getQuestState());
        uiStage.addActor(questTracker);
        
        questTracker.setX(width - 200f);
        questTracker.setY(height - 5f);
        
        final AddItemListener questItemListener = new AddItemListener() {
            @Override
            public void afterItemAdded(final ItemDetails item) {
                final List<QuestDetails> currentQuests = questState.getCurrentQuests();
                for (final QuestDetails quest : currentQuests) {
                    final QuestItem questItem = quest.findItem(item);
                    if (questItem == null) {
                        continue;
                    }
                    
                    questState.registerItemTaken(questItem);
                    questTracker.updateSteps();
                }
            }
        };
        
        playerState.getInventory().addInventoryListener(questItemListener);
    }
    ...
    private void performAction() {
        ...
        final QuestItem questItem = currentMap.getNearbyQuestItems(player);
        if (questItem != null) {
            final ItemDetails itemDetails = questItem.getItem();
            playerState.getInventory().addQuestItem(itemDetails);
//            questState.registerItemTaken(questItem);
        }
    ...
    private void displayQuestDialog(final QuestDetails questDetails) {
        ...
        acceptQuest.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                questTracker.setCurrentQuest(questDetails);
                currentMap.addQuest(questDetails);
                dialog.setVisible(false);
    ...
    private void initializeMap(final String nextMap, final String prevMap, final Integer mapIndex, final float deltaTime) {
        for (final MapDetails md : currentArea.getMaps()) {
            mapManager.addMap(md.getName(), "gfx/maps/" + md.getRelativePath(), npcDetails, md, questManager, questState);
        }
